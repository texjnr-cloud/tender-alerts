...
async function qualifyTender(tender, builderProfile) {
  ...
  // Check if tender location matches user location area
  const userLocation = builderProfile.location.toLowerCase();
  const tenderLocation = tender.location.toLowerCase();
  const westMidlandsAreas = ['birmingham', 'dudley', 'sandwell', 'walsall', 'wolverhampton', 'solihull', 'coventry'];
  
  // If user is in West Midlands, only show West Midlands tenders
  if (westMidlandsAreas.includes(userLocation)) {
    if (!westMidlandsAreas.includes(tenderLocation)) {
      // Automatically reject tenders outside area
      return {
        tender,
        status: 'red',
        passes: [],
        issues: ['Tender location outside your area'],
        score: -999
      };
    }
  }
  
  // Rest of existing qualification code...
export async function qualifyTender(tender, builderProfile) {
  const description = (tender.title + ' ' + tender.description).toLowerCase();
...
async function sendEmailAlert(user, matches) {
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Tender Alerts <hello@tendrly.co.uk>',
        to: user.email,
...
async function runDailyTenderCheck() {
