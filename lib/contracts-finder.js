...
export async function fetchAdaptationTenders() {
  ...
  try {
      ...
      if (data.releases) {
        const tenders = data.releases
          .map(release => ({
            ...
            noticeId: release.ocid.split('-')[0], // Extract notice ID from OCID
            location: location
          }))
          .filter(tender => {
    ...
    console.log(`Fetched ${allTenders.length} unique adaptation tenders from West Midlands councils`);
    return allTenders;
...
// ==========================================
// 2. FILTER FOR ACTUAL ADAPTATION TENDERS
// ==========================================
...
// ==========================================
// 3. QUALIFY TENDER AGAINST USER PROFILE
// ==========================================
...
export async function qualifyTender(tender, profile) {
  ...
  // Build the correct Contracts Finder URL
  // Format: https://www.contractsfinder.service.gov.uk/Notice/{noticeId}
  const tenderUrl = `https://www.contractsfinder.service.gov.uk/Notice/${tender.noticeId}`;
